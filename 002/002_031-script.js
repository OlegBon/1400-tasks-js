...
input.placeholder = "Введіть скільки конфет, печива та яблук"; // Плейсхолдер для поля ввода
let button = document.createElement("button");
button.textContent = "Обчислити покупку"; // Текст кнопки
...
const arrIn = ["x", "y", "z"]; // Масив назв змінних для виводу
// Ціни на товари: конфети, печиво та яблука
const prices = {
  candy: 120,
  cookie: 80,
  apple: 45,
...
// Клас Singleton для обчислення вартості покупки
class PriceCalculator {
  ...
  calculateTotalPrice(candyKg, cookieKg, appleKg) {
    if (candyKg < 0 || cookieKg < 0 || appleKg < 0) {
    ...
    return (
      candyKg * prices.candy +
      cookieKg * prices.cookie +
      appleKg * prices.apple
    ).toFixed(2);
  }
}
...
function handlerClick() {
  ...
  if (nmbr.length === 3) {
    try {
      let result = new PriceCalculator().calculateTotalPrice(
        nmbr[0],
        nmbr[1],
        nmbr[2]
      );
      outputParagraph.textContent = `Введено x = ${nmbr[0]}, y = ${nmbr[1]}, z = ${nmbr[2]}. Результат: ${result} грн.`;
    } catch (error) {
