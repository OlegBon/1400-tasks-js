...
button.textContent = "Обчислити гіпотенузу"; // Текст кнопки
...
// Обчислення гіпотенузи прямокутного трикутника
function calculateHypotenuse(cathetus1, cathetus2) {
  ...
  return Math.sqrt(cathetus1 * cathetus1 + cathetus2 * cathetus2).toFixed(2); // Обчислення гіпотенузи
}
...
button.addEventListener("click", function () {
  ...
  if (nmbr.length === 2) {
    try {
      let hypotenuse = calculateHypotenuse(nmbr[0], nmbr[1]);
      outputParagraph.textContent = `Введено ${arrIn[0]} = ${nmbr[0]}, ${arrIn[1]} = ${nmbr[1]}. Гіпотенуза = ${hypotenuse} см`;
    } catch (error) {
