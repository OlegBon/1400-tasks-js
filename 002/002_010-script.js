...
// Середнє арифметичне та середне геометричне
class AverageAndGeometricMean {
  #x;
  #y;
  constructor(x, y) {
    this.#x = x;
...
button.addEventListener("click", function () {
  ...
  if (nmbr.length === 2) {
    try {
      let avrg = new AverageAndGeometricMean(nmbr[0], nmbr[1]);
      outputParagraph.textContent = `Введено x = ${nmbr[0]}, y = ${nmbr[1]}. Результат: середнє арифметичне = ${avrg.average}, середне геометричне = ${avrg.geometric}`;
    } catch (error) {
