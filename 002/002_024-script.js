...
// Клас для обчислювання суми, різниці, добутка, а також частки від ділення першого числа на друге
class AverageCalculator {
  ...
  calculateAverage() {
    return this.a + this.b; // Сума
  }

  calculateGeometricMean() {
    return this.a - this.b; // Різниця
  }

  calculateProduct() {
    return this.a * this.b; // Добуток
  }

  calculateQuotient() {
    if (this.b === 0) {
      throw FormulaError.divisionByZero(arrIn[1], this.b);
    }
    return this.a / this.b; // Частка
  }
}
...
button.addEventListener("click", function () {
  ...
  if (nmbr.length === 2) {
    try {
      ...
      let product = calculator.calculateProduct();
      let quotient = calculator.calculateQuotient();
      outputParagraph.textContent = `Введено: ${arrIn[0]} = ${nmbr[0]}, ${arrIn[1]} = ${nmbr[1]}. Результати: сума = ${average}, різниця = ${geometricMean}, добуток = ${product}, частка = ${quotient}`;
    } catch (error) {
